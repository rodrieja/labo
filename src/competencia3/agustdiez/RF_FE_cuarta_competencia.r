...
#Parametros del script
PARAM  <- list()
PARAM$experimento  <- "RF_DR9140"

PARAM$exp_input  <- "RF_CA9060"
...
#Parametros del script
PARAM  <- list()
PARAM$experimento <- "RF_FE9250"

PARAM$exp_input  <- "RF_DR9140"
...
AgregaVarRandomForest  <- function( num.trees, max.depth, min.node.size, mtry)
{
  ...
  dataset_rf[ , entrenamiento := as.integer( ((foto_mes >= 201905 & foto_mes <= 201912) | (foto_mes >= 202011 & foto_mes <= 202105)) & ( clase01==1 | azar < 0.10 )) ]
...
#grabo el dataset
fwrite( dataset,
        "dataset_lags2_tendencia3_RF.csv.gz",
        logical01= TRUE,
        sep= "," )
