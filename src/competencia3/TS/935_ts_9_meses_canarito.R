...
PARAM <- list()
PARAM$experimento <- "TS9282"

PARAM$exp_input <- "FE9282"

PARAM$future <- c(202107)

PARAM$final_train <- c(202009, 202010, 202011, 202012, 202101, 202102, 202103, 202104, 202105)

PARAM$train$training <- c(202004, 202005, 202006, 202007, 202011, 202012, 202101, 202102, 202103)
...
dataset_input <- paste0("./exp/", PARAM$exp_input, "/dataset_canaritos.csv.gz")
...
fwrite(dataset[foto_mes %in% PARAM$future, ],
       file = "dataset_canarito_9_future.csv.gz",
       logical01 = TRUE,
       sep = ","
)
...
fwrite(dataset[foto_mes %in% PARAM$final_train, ],
       file = "dataset_canarito_9_train_final.csv.gz",
       logical01 = TRUE,
       sep = ","
)
...
fwrite(dataset[fold_train + fold_validate + fold_test >= 1, ],
       file = "dataset_canarito_9_training.csv.gz",
       logical01 = TRUE,
       sep = ","
)
